...
.node-card {
  ...
  transition: opacity 0.3s ease, transform 0.3s ease, box-shadow 0.2s cubic-bezier(0.4, 0, 0.2, 1), border-color 0.2s cubic-bezier(0.4, 0, 0.2, 1);
  cursor: grab;
}
.node-card:hover { transform: translateY(-2px); border-color: rgba(56, 189, 248, 0.35); box-shadow: 0 8px 24px rgba(0, 0, 0, 0.3), 0 0 15px rgba(56, 189, 248, 0.1); }
.node-card:active { cursor: grabbing; }
.node-card.is-dragging { opacity: 0.35; transform: scale(0.97); border: 2px dashed rgba(56, 189, 248, 0.6) !important; box-shadow: 0 8px 24px rgba(0, 0, 0, 0.4); cursor: grabbing; }
.node-card.is-drag-over { border-color: #38bdf8 !important; box-shadow: 0 0 0 2px rgba(56, 189, 248, 0.5), 0 8px 25px rgba(56, 189, 248, 0.25) !important; transform: translateY(-4px) scale(1.02); background: rgba(56, 189, 248, 0.08); }
.node-overloaded { border-color: rgba(244, 63, 94, 0.4); box-shadow: 0 4px 20px rgba(244, 63, 94, 0.12); }
.node-warning { border-color: rgba(245, 158, 11, 0.3); }
.node-down { opacity: 0.6; filter: grayscale(0.7); }
.is-busiest { box-shadow: 0 0 25px rgba(245, 158, 11, 0.18); border-color: rgba(245, 158, 11, 0.4); }
.drag-handle { display: inline-flex; align-items: center; justify-content: center; color: var(--text-muted, #94a3b8); font-size: 13px; letter-spacing: -2px; cursor: grab; padding: 2px 4px; border-radius: 4px; transition: all 0.15s ease; user-select: none; opacity: 0.5; }
.node-card:hover .drag-handle { opacity: 0.95; color: var(--text-primary, #f8fafc); background: rgba(255, 255, 255, 0.06); }
.drag-handle:hover { opacity: 1; background: rgba(56, 189, 248, 0.15); color: #38bdf8; }
.drag-handle:active { cursor: grabbing; }
.badge-traffic-pulse { animation: pulse-traffic 2s infinite ease-in-out; }
@keyframes pulse-traffic { 0%, 100% { transform: scale(1); opacity: 1; } 50% { transform: scale(1.05); opacity: 0.85; } }
.node-card-header { display: flex; flex-direction: column; gap: 8px; width: 100%; }
.node-header-top { display: flex; align-items: center; justify-content: space-between; gap: 8px; width: 100%; }
.node-identity { display: flex; align-items: center; gap: 8px; min-width: 0; flex: 1; }
...
.status-dot-pulse { animation: pulseDot 2s infinite ease-in-out; }
@keyframes pulseDot { 0%, 100% { opacity: 1; } 50% { opacity: 0.5; } }
.node-name { font-size: 1.1rem; font-weight: 700; letter-spacing: -0.01em; color: var(--text-primary, #f8fafc); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin: 0; line-height: 1.2; }
.node-header-badges { display: flex; align-items: center; gap: 6px; flex-shrink: 0; }
.badge { display: inline-flex; align-items: center; padding: 2px 8px; border-radius: 6px; font-size: 10.5px; font-weight: 700; letter-spacing: 0.04em; text-transform: uppercase; }
.badge-offline { background: rgba(244, 63, 94, 0.18); color: #f43f5e; border: 1px solid rgba(244, 63, 94, 0.4); }
...
.node-header-meta { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
.badge-meta { display: inline-flex; align-items: center; gap: 4px; font-size: 0.72rem; font-weight: 600; padding: 2px 7px; border-radius: 6px; background: rgba(0, 0, 0, 0.3); border: 1px solid rgba(255, 255, 255, 0.08); color: var(--text-secondary, #94a3b8); white-space: nowrap; }
.node-card-footer { margin-top: auto; }
.node-footer-actions { display: flex; align-items: center; gap: 8px; width: 100%; }
.btn-node-action { flex: 1; display: inline-flex; align-items: center; justify-content: center; gap: 6px; padding: 7px 12px; border-radius: 8px; background: rgba(255, 255, 255, 0.04); border: 1px solid rgba(255, 255, 255, 0.09); color: var(--text-secondary, #94a3b8); font-size: 11.5px; font-weight: 600; cursor: pointer; transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1); white-space: nowrap; }
.btn-node-action:hover { background: rgba(255, 255, 255, 0.09); color: var(--text-primary, #f8fafc); border-color: rgba(56, 189, 248, 0.4); transform: translateY(-1px); }
.btn-inspect-node { border-color: rgba(6, 182, 212, 0.25); color: #22d3ee; background: rgba(6, 182, 212, 0.06); }
.btn-inspect-node:hover { background: rgba(6, 182, 212, 0.16); border-color: #06b6d4; color: #38bdf8; box-shadow: 0 0 12px rgba(6, 182, 212, 0.25); transform: translateY(-1px); }
.btn-manage-host { border-color: rgba(129, 140, 248, 0.25); color: #a5b4fc; background: rgba(99, 102, 241, 0.06); }
.btn-manage-host:hover { border-color: #818cf8; color: #c7d2fe; background: rgba(99, 102, 241, 0.16); box-shadow: 0 0 12px rgba(99, 102, 241, 0.25); transform: translateY(-1px); }
