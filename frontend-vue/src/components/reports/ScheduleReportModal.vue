...
const activeCronExpression = computed(() => {
  if (cadencePreset.value === 'daily') return '0 6 * * *'
  if (cadencePreset.value === 'monthly') return '0 0 1 * *'
  if (cadencePreset.value === 'custom') return customCron.value.trim() || '0 8 * * 1'
  return '0 8 * * 1'
})
...
function handleSave() {
  ...
  let cron = activeCronExpression.value
    ...
    <form @submit.prevent="handleSave" class="form-layout">
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          ...
          <div class="format-options-group" role="radiogroup" aria-label="Export Format">
            <button
              type="button"
              class="format-checkbox-pill"
      ...
      <div class="form-group">
        <label>Automated Cadence (Schedule Cron Picker)</label>
        <div class="cron-picker-wrap">
          <div class="cron-presets-grid">
            <button
              type="button"
              class="cron-preset-btn"
              :class="{ active: cadencePreset === 'weekly' }"
              @click="cadencePreset = 'weekly'"
            >
              Weekly
            </button>
            <button
              type="button"
              class="cron-preset-btn"
              :class="{ active: cadencePreset === 'daily' }"
              @click="cadencePreset = 'daily'"
            >
              Daily
            </button>
            <button
              type="button"
              class="cron-preset-btn"
              :class="{ active: cadencePreset === 'monthly' }"
              @click="cadencePreset = 'monthly'"
            >
              Monthly
            </button>
            <button
              type="button"
              class="cron-preset-btn"
              :class="{ active: cadencePreset === 'custom' }"
              @click="cadencePreset = 'custom'"
            >
              Custom Cron
            </button>
          </div>
      ...
      <div class="form-row">
        <div class="form-group" v-if="cadencePreset === 'custom'">
          <label>Custom Cron Expression (5-Field)</label>
          <input 
            v-model="customCron" 
            type="text" 
      ...
      <div class="form-group recipient-list-container">
