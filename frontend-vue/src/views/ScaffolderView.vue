...
const defaultTemplates: Template[] = [
  {
    ...
    tags: ['golang', 'chi', 'microservice', 'api', 'kubernetes'],
    variables: [
      ...
      { name: 'environment', label: 'Environment', type: 'select', default: 'production', required: false, options: ['development', 'staging', 'production'] },
    ],
...
data:
  APP_ENV: "{{.environment}}"
  PORT: "{{.port}}"`,
    helm_values: `nameOverride: "{{.app_name}}"
...
services:
  ...
  },
  {
    ...
    built_in: true,
    tenant_id: 'default-tenant',
    tags: ['nodejs', 'fastify', 'typescript', 'api', 'rest'],
    variables: [
...
services:
  ...
  },
  {
    ...
    built_in: true,
    tenant_id: 'default-tenant',
    tags: ['python', 'fastapi', 'uvicorn', 'async', 'api'],
    variables: [
...
services:
  ...
  },
  {
    ...
    built_in: true,
    tenant_id: 'default-tenant',
    tags: ['nginx', 'proxy', 'web', 'frontend', 'ingress'],
    variables: [
...
services:
  ...
  },
  {
    ...
    built_in: true,
    tenant_id: 'default-tenant',
    tags: ['postgres', 'sql', 'database', 'statefulset', 'storage'],
    variables: [
...
stringData:
  ...
  password: "{{.db_password}}"`,
    helm_values: `nameOverride: "{{.db_name}}"
image:
  repository: postgres
  tag: 16-alpine
auth:
...
async function loadTemplates() {
  ...
  try {
    ...
    let res: Template[] = []
    try {
      res = await scaffoldApi.list(filter)
    } catch {
      // Fallback
    }
    if (!res || res.length === 0) {
      let defs = [...defaultTemplates]
      if (filter.category) {
        defs = defs.filter(t => t.category === filter.category)
      }
      if (filter.search) {
