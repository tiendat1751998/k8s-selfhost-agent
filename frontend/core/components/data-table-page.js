    ...
    return {
      ...
      async loadData() {
        ...
        try {
          ...
          if (area) {
             if (config.viewType === 'grid') {
               area.innerHTML = \`<div class="text-center text-danger w-100 py-10" style="grid-column:1/-1;">Failed to load data: \${e.message}</div>\`;
             } else {
               area.innerHTML = \`<tr><td colspan="\${(config.columns || []).length}" class="text-center text-danger">Failed to load data: \${e.message}</td></tr>\`;
             }
          }
        }
      },
      ...
      renderContent(data) {
        ...
        if (!data || data.length === 0) {
          if (config.viewType === 'grid') {
             area.innerHTML = \`<div class="text-center text-muted w-100 py-10" style="grid-column:1/-1;">\${config.emptyMessage || 'No items found.'}</div>\`;
          } else {
             area.innerHTML = \`
              <tr>
                <td colspan="\${(config.columns || []).length}" style="text-align:center;color:var(--color-muted);padding:var(--space-lg);">
                  <div style="font-size:36px;margin-bottom:var(--space-sm);">\${config.emptyIcon || 'dY"<'}</div>
                  <h4 style="margin:0;font-weight:600;">\${config.emptyMessage || 'No items available'}</h4>
                </td>
              </tr>
            \`;
          }
