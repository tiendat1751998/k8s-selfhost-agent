...
(function (global) {
  ...
  async function init() {
    ...
    if (global.ViewLoader) {
      await global.ViewLoader.loadTemplate('/modules/clusters/template.html');
    }

    tableBody = document.getElementById('k8s-table-body');
    emptyEl = document.getElementById('k8s-empty');
    addBtn = document.getElementById('add-cluster-btn');
  ...
  async function testConnection(name) {
    try {
      await APIClient.post('/fleet/' + name + '/test');
      ...
      actions: [
        ...
        { label: 'Add Cluster', primary: true, closeOnClick: false, onClick: async function () {
          ...
          try {
            ...
            alert('Error adding cluster: ' + e.message);
          }
        }}
      ]
    });
